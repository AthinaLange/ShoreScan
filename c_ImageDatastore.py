...
# ------------- Folder Selection ------------- 
    ...
    def select_folder():
        """
        Open a dialog box to select a folder and return the selected path.
...
# ------------- Image Loading and Metadata Parsing ------------- 
    def load_images(self):
        """
        Traverse the folder structure to load image file paths and associated metadata
        (timestamp, camera type, image type, etc.) into the datastore.
        """
...
# ------------- Copy images to folder
    def copy_images_to_folder(self, destination_folder = None, hierarchical = False):
        """
        Copy all images from the datastore to the specified destination folder.
        If no destination folder is specified, save images to their original directory.

        Args:
        - destination_folder (str): The path to the destination folder where images will be copied.
        - hierarchical (bool): If True, organize images into subfolders by site, camera, year, month, and image type.
                                If False, copy all images into a single folder.

        Raises:
        - ValueError: If the destination folder is not specified or invalid (when not saving to original folder).
        """
 ...
 # ------------- Image Listing ------------- 
    def list_images(self, site = None, camera = None, year = None, month = None):
        """
        List images optionally filtered by site, camera, year, and month.
        Args:
        - site (str, optional): Site identifier.
        - camera (str, optional): Camera identifier.
        - year (str, optional): Year.
...
# ------------- Image Statistics ------------- 
    def image_stats(self):
        """
        Display statistics about the images stored in the datastore.
        """
        total_images = 0
        days_set = set()
...
# ------------- Plotting -------------  
    def _validate_year_and_index(self, year, index):
        ...
        Args:
            year (str): The year of the images.
            index (int): The index of the image within the specified year.

        Returns:
            dict: The image metadata if valid, else None.
        """
        if not self.images:
    ...
    def plot_image(self, image_metadata):
        ...
        Args:
            image_metadata (dict): A dictionary containing the image's metadata.
        """
...
# ------------- Camera Site Info ------------- 
    def load_camera_sites(self, csv_file = 'camera_sites.csv'):
        ...
        Args:
        - file_path (str): The path to the CSV file containing camera site data.

        Returns:
        - dict: A dictionary with site names as keys and tuples (lat, lon, camera_angle) as values.
        """
...
# ------------- Filters -------------
    ...
    def filter_blurry_images(self, blur_thresholds = None, image_type = None):
        ...
        Args:
        - blur_thresholds (dict, optional): A dictionary where keys are image types 
                                            (e.g., 'bright', 'snap') and values are 
                                            the Laplacian variance thresholds below 
                                            which images are considered blurry.
                                            If not provided, the default threshold will be used.
        - image_type (str, optional): If specified, only filter images of this type 
                                    (e.g., 'bright', 'snap').
